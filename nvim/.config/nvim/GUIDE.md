# ME Kickstart — Neovim Config Guide
...
│   └── plugins/
│       ├── colorscheme.lua         # Color Scheme(kangawa(default))
│       ├── snacks.lua              # Snacks.nvim (picker, explorer, UI)
│       ├── completion.lua          # LuaSnip.nvim, AI completion paired with BLink.nvim
│       ├── git.lua                 # git signs, blame and diff
│       ├── editor.lua              # Editor utilites
│       │── ui.lua                  # UI utilites: which-key, mini.nvim, etc.
│       └── syntax.lua              # Treesitter.nvim
```
...
| `<Leader>tf` | Tggole floating terminal |
...
| `https://github.com/MeanderingProgrammer/render-markdown.nvim` | Renders markdown in the terminal |
...
### AI // Currently not in use, the plugins are removed
...
- **Notifications**: Toast notifications appear via Snacks notifier. Run `:lua Snacks.notifier.show_history()` to see past
mssages.
